...
export async function onRequestPost(context) {
  try {
    ...
    if (!apiKey) return json({ ok:false, error:'Contact service is not configured.' }, 503);

    const toEmail = context.env.CONTACT_TO_EMAIL || 'enquiry@blutech.io';
    const fromEmail = context.env.CONTACT_FROM_EMAIL || 'Blutech Website <onboarding@resend.dev>';
    ...
    if (!resend.ok) {
      ...
      console.error('Resend API error', resend.status, detail);
