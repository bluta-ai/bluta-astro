...
export type SiteLanguage = "en" | "tc" | "sc" | "ar";
...
export function LanguageProvider({ children, initialLanguage = "en" }: { children: React.ReactNode; initialLanguage?: SiteLanguage }) {
  ...
  useEffect(() => {
    document.documentElement.lang = language === "en" ? "en" : language === "tc" ? "zh-Hant" : language === "sc" ? "zh-Hans" : "ar";
...
export function localePath(path: string, language: SiteLanguage) {
  const clean = path.replace(/^\/(zh-hant|zh-hans|ar)(?=\/|$)/, "") || "/";
  const prefix = language === "tc" ? "/zh-hant" : language === "sc" ? "/zh-hans" : language === "ar" ? "/ar" : "";
...
export function Localized({ en, tc, sc, ar }: { en: React.ReactNode; tc: React.ReactNode; sc: React.ReactNode; ar?: React.ReactNode }) {
  const { language } = useSiteLanguage();
  return <>{language === "tc" ? tc : language === "sc" ? sc : language === "ar" ? (ar ?? en) : en}</>;
}
...
export function LanguageSelector({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={compact ? "mobile-language global-language" : "language global-language"} aria-label="Language selector">
    {(["en", "tc", "sc", "ar"] as SiteLanguage[]).map((item) => <a key={item} href={localePath(pathname, item)} hrefLang={item === "tc" ? "zh-Hant" : item === "sc" ? "zh-Hans" : item} lang={item === "tc" ? "zh-Hant" : item === "sc" ? "zh-Hans" : item} className={language === item ? "active" : ""}>{item === "en" ? "EN" : item === "tc" ? (compact ? "繁體" : "繁") : item === "sc" ? (compact ? "简体" : "简") : (compact ? "العربية" : "AR")}</a>)}
  </div>;
}
