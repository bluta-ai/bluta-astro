    ...
    <main className="internal-page product-detail-page">
      ...
      <section className="detail-hero">
        <div className="detail-copy">
          ...
          <div className="detail-actions">
            <a className="button button-light" href={`mailto:enquiry@blutech.io?subject=${encodeURIComponent(`${product.model} ${product.name} enquiry`)}`}><Localized en="Discuss this product" tc="查詢此產品" sc="咨询此产品" ar="ناقش هذا المنتج" /> <Arrow /></a>
            <LocalizedLink className="text-link light-link" href="/products"><Localized en="All products" tc="所有產品" sc="所有产品" ar="كل المنتجات" /> <Arrow /></LocalizedLink>
          </div>
        </div>
        ...
        {product.image ? (
          ...
          <div className="detail-media system-visual" aria-label={`${product.model} functional signal path`}>
            <span className="system-visual-label"><Localized en="FUNCTIONAL VIEW" tc="功能視圖" sc="功能视图" ar="عرض وظيفي" /> · {product.model}</span>
            ...
            <p><Localized en="Functional diagram — request the current product submittal for exact enclosure and installation details." tc="功能示意圖——外殼及安裝細節以最新產品提交文件為準。" sc="功能示意图——外壳及安装细节以最新产品提交文件为准。" ar="مخطط وظيفي — اطلب وثيقة المنتج الحالية لتأكيد تفاصيل الهيكل والتركيب." /></p>
          </div>
        )}
      </section>

      <nav className="product-subnav" aria-label="Product page sections"><a href="#overview"><Localized en="Overview" tc="概覽" sc="概览" ar="نظرة عامة" /></a><a href="#capabilities"><Localized en="Capabilities" tc="功能" sc="功能" ar="القدرات" /></a>{product.facts && product.facts.length > 0 && <a href="#specifications"><Localized en="Specifications" tc="規格" sc="规格" ar="المواصفات" /></a>}</nav>

      <section id="overview" className="product-value section-pad">
        <div className="product-value-heading">
          <p className="section-label"><Localized en="Product benefits" tc="產品效益" sc="产品效益" ar="فوائد المنتج" /></p>
          <h2><Localized en="What this product improves." tc="這項產品帶來甚麼改善。" sc="这项产品带来什么改善。" ar="ما الذي يحسّنه هذا المنتج؟" /></h2>
          <p>{product.description}</p>
        </div>
        <div className="product-benefit-list">
          <article><h3><Localized en="Outcome for the property" tc="物業成果" sc="物业成果" ar="النتيجة للعقار" /></h3><p>{product.outcome}</p></article>
          <article><h3><Localized en="Value for operations" tc="營運價值" sc="运营价值" ar="القيمة للتشغيل" /></h3><p>{product.features[0]?.body}</p></article>
          <article><h3><Localized en="How it fits the project" tc="如何配合項目" sc="如何配合项目" ar="ملاءمته للمشروع" /></h3><p>{product.deployment}</p></article>
        </div>
        <div className="product-context-row"><div><h3><Localized en="Installation approach" tc="安裝方式" sc="安装方式" ar="نهج التركيب" /></h3><p>{product.installation}</p></div><div><h3><Localized en="Best suited to" tc="適用場景" sc="适用场景" ar="الأنسب لـ" /></h3><ul>{product.applications.map((application) => <li key={application}>{application}</li>)}</ul></div></div>
      </section>

      <section id="capabilities" className="product-capabilities section-pad">
        <div className="section-intro compact-intro">
          <div><p className="section-label"><Localized en="Core capabilities" tc="主要功能" sc="主要功能" ar="القدرات الأساسية" /></p><h2><Localized en="Built for a specific building outcome." tc="每項功能都對應實際樓宇成果。" sc="每项功能都对应实际楼宇成果。" ar="مصمم لتحقيق نتيجة واضحة في المبنى." /></h2></div>
          <p><Localized en="Each capability supports clearer information, faster response, better use of space or simpler operations." tc="功能聚焦於更清晰的資訊、更快回應、更有效運用空間與更簡單的營運。" sc="功能聚焦于更清晰的信息、更快响应、更有效利用空间与更简单的运营。" ar="تدعم كل قدرة معلومات أوضح واستجابة أسرع واستخداماً أفضل للمساحة وعمليات أبسط." /></p>
        </div>
      ...
      {product.facts && product.facts.length > 0 && (
        <section id="specifications" className="product-fact-sheet section-pad">
          <div><p className="section-label"><Localized en="Documented specifications" tc="已確認資料" sc="已确认资料" ar="المواصفات الموثقة" /></p><h2><Localized en="Confirmed product facts." tc="目前已確認的產品規格。" sc="目前已确认的产品规格。" ar="حقائق المنتج المؤكدة." /></h2></div>
      ...
      {related.length > 0 && (
        <section className="related section-pad">
          <p className="eyebrow"><Localized en="SAME PRODUCT FAMILY" tc="同一產品系列" sc="同一产品系列" ar="من عائلة المنتج" /></p><h2><Localized en="Related products." tc="相關產品。" sc="相关产品。" ar="منتجات ذات صلة." /></h2>
          <div>{related.map((item) => <LocalizedLink key={item.slug} href={`/products/${item.slug}`}><span>{item.model}</span><h3>{item.name}</h3><p>{item.outcome}</p><b><Localized en="View product" tc="查看產品" sc="查看产品" ar="عرض المنتج" /> <Arrow /></b></LocalizedLink>)}</div>
        </section>
      )}
