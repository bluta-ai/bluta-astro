    ...
    <main className="internal-page">
      ...
      <section className="page-hero products-hero">
        <div className="page-hero-copy">
          <p className="eyebrow light"><Localized en="VERIFIED PRODUCT CATALOGUE" tc="已驗證產品目錄" sc="已验证产品目录" ar="كتالوج منتجات موثّق" /></p>
          <h1><Localized en={<>Built for the realities<br />of a live building.</>} tc={<>為真實營運中的建築<br />而設計。</>} sc={<>为真实运营中的建筑<br />而设计。</>} ar={<>مصمم لواقع<br />المباني العاملة.</>} /></h1>
          <p><Localized en="Purpose-built sensors, gateways and interfaces designed around property outcomes—not novelty. Every model shown here is drawn from Blutech's current, proven catalogue." tc="感測器、網關與資訊介面均以物業成果為中心，而非追求新奇。頁面所列型號均來自 Blutech 現行並經實際應用的產品目錄。" sc="传感器、网关与信息界面均以物业成果为中心，而非追求新奇。页面所列型号均来自 Blutech 现行并经实际应用的产品目录。" ar="مستشعرات وبوابات وواجهات مصممة حول نتائج العقار، وليست لمجرد الحداثة. كل طراز معروض من كتالوج Blutech الحالي والمثبت." /></p>
          <a className="button button-light" href="#catalogue"><Localized en="Explore the range" tc="查看產品系列" sc="查看产品系列" ar="استكشف المجموعة" /> <Arrow /></a>
        </div>
      ...
      <section className="product-principles section-pad">
        <div className="section-intro">
          <div><p className="section-label"><Localized en="Designed as a system" tc="以完整系統思維設計" sc="以完整系统思维设计" ar="مصمم كنظام متكامل" /></p><h2><Localized en={<>Quiet hardware.<br />Useful signals.</>} tc={<>低調硬件。<br />實用訊號。</>} sc={<>低调硬件。<br />实用信号。</>} ar={<>أجهزة هادئة.<br />إشارات مفيدة.</>} /></h2></div>
        ...
        <div className="principle-grid">
          <article><span>01</span><h3><Localized en="Accurate sensing" tc="精準感測" sc="精准感知" ar="استشعار دقيق" /></h3><p><Localized en="Start with the signal the building actually needs, then select thermal, mmWave, contact, environmental or spatial sensing around that outcome." tc="先確認建築真正需要的訊號，再按成果選擇熱能、毫米波、接點、環境或空間感測。" sc="先确认建筑真正需要的信号，再按成果选择热能、毫米波、触点、环境或空间感知。" ar="ابدأ بالإشارة التي يحتاجها المبنى فعلاً، ثم اختر الاستشعار الحراري أو الموجات المليمترية أو البيئي أو المكاني." /></p></article>
          <article><span>02</span><h3><Localized en="Retrofit ready" tc="適合改造" sc="适合改造" ar="جاهز للتحديث" /></h3><p><Localized en="Wireless-first options reduce disruption in operating properties while gateways and open interfaces connect what is already there." tc="無線優先方案減少對營運中物業的干擾，網關與開放介面則連接既有系統。" sc="无线优先方案减少对运营中物业的干扰，网关与开放接口则连接既有系统。" ar="تقلل الخيارات اللاسلكية أثر التنفيذ في العقارات العاملة، بينما تربط البوابات والواجهات المفتوحة الأنظمة القائمة." /></p></article>
          <article><span>03</span><h3><Localized en="Operationally clear" tc="營運清晰" sc="运营清晰" ar="وضوح تشغيلي" /></h3><p><Localized en="Product selection begins with the alarm, workflow, report or occupant experience the team needs to improve." tc="由團隊需要改善的警報、流程、報告或使用者體驗出發選擇產品。" sc="从团队需要改善的警报、流程、报告或使用者体验出发选择产品。" ar="يبدأ اختيار المنتج من التنبيه أو سير العمل أو التقرير أو تجربة المستخدم التي يحتاج الفريق إلى تحسينها." /></p></article>
        </div>
      </section>

      <section className="featured-products dark-section section-pad">
        <div className="catalogue-heading"><div><p className="section-label light"><Localized en="Core range" tc="核心產品" sc="核心产品" ar="المجموعة الأساسية" /></p><h2><Localized en="Start with the outcome." tc="由成果開始。" sc="从成果开始。" ar="ابدأ بالنتيجة." /></h2></div><p><Localized en="These products form the core of Blutech deployments across offices, aviation, healthcare, government and retail." tc="這些產品構成 Blutech 在辦公室、航空、醫療、政府及零售項目的核心。" sc="这些产品构成 Blutech 在办公室、航空、医疗、政府及零售项目的核心。" ar="تشكل هذه المنتجات أساس مشاريع Blutech في المكاتب والطيران والرعاية الصحية والقطاع الحكومي والتجزئة." /></p></div>
        <div className="featured-product-grid">
          {featured.map((product) => (
            <LocalizedLink className="featured-product" key={product.slug} href={`/products/${product.slug}`}>
              ...
              <div><p>{product.model} · {product.category}</p><h3>{product.name}</h3><span>{product.summary}</span><b><Localized en="View product" tc="查看產品" sc="查看产品" ar="عرض المنتج" /> <Arrow /></b></div>
            </LocalizedLink>
          ))}
        </div>
      </section>
        ...
        {categories.map((category, categoryIndex) => {
          ...
          return (
            <div className="catalogue-family" key={category}>
              <div className="family-title"><span>{String(categoryIndex + 1).padStart(2, "0")}</span><h3>{localizeCategory(category, locale)}</h3><b>{family.length} <Localized en={family.length === 1 ? "product" : "products"} tc="款產品" sc="款产品" ar="منتج" /></b></div>
